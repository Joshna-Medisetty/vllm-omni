...
class AutoencoderKL(nn.Module):
    ...
    def tiled_decode(self, z: torch.Tensor, return_dict: bool = True):
        """Decode latents tile-by-tile to reduce peak memory usage.

        Splits the latent tensor into overlapping spatial tiles, decodes each
        tile independently, then blends the overlapping regions using
        blend_v / blend_h helpers.
        """
        overlap_size = int(self.tile_latent_min_size * (1 - self.tile_overlap_factor))
        blend_extent = int(self.tile_sample_min_size * self.tile_overlap_factor)
        row_limit = self.tile_sample_min_size - blend_extent

        # Collect decoded rows of tiles
        rows = []
